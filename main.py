...
import TMSC_m7G
from model_operation import save_model, adjust_model,load_model
import torch
...
def load_data(config):
    ...
    ind_iter = data_loader.load_data(config.path_ind_data,config)
    bench_iter = data_loader.load_data(config.path_bench_data,config)
    config.max_len = 256
...
def periodic_valid(valid_iter, model, criterion, config, sum_epoch):
    ...
    return valid_metric, valid_loss, valid_repres_list, valid_label_list,valid_roc_data, valid_prc_data


def train_m7G(train_iter, valid_iter, test_iter, model, optimizer, criterion, config, iter_k):
    steps = 0
    best_acc = 0
    best_performance = 0
    for epoch in range(1, config.epoch + 1):
        ...
        for batch in train_iter:
            ...
            logits, output,logits_base = model(input, origin_input,pos_embed)
            ...
            if steps % config.interval_log == 0:
                ...
                sys.stdout.write(
                    '\rEpoch[{}] Batch[{}] - loss: {:.6f} | ACC: {:.4f}%({}/{})'.format(epoch, steps, loss, train_acc, corrects, the_batch_size))
                print()
        ...
        if valid_iter and sum_epoch % config.interval_valid == 0:
            valid_metric, valid_loss, valid_repres_list, valid_label_list,valid_roc_data, valid_prc_data = periodic_valid(valid_iter, model, criterion, config, sum_epoch)
        ...
        if test_iter and sum_epoch % config.interval_test == 0:
            ...
            test_metric, test_loss, test_repres_list, test_label_list,test_roc_data, test_prc_data = periodic_test(test_iter, model, criterion, config, sum_epoch)
            '''Periodic Save'''
            ...
            #     visualization.rep_plt(repres_list, y, epoch)
...
def model_eval(data_iter, model, criterion, config):
    ...
    with torch.no_grad():
        for batch in data_iter:
            input, origin_inpt,pos_embed, label = batch
            logits, output,logits_base = model(input, origin_inpt,pos_embed)
            repres_list.extend(output.cpu().detach().numpy())
            label_list.extend(label.cpu().detach().numpy())
...
def k_fold_CV(train_iter_orgin, test_iter, config):
    ...
    for iter_k in range(config.k_fold):
        ...
        valid_performance = train_m7G(train_iter, valid_iter, test_iter, model, optimizer, criterion, config, iter_k)[0]
        print('=' * 50 + 'Train Finished' + '=' * 50)
        # if config.save_best:
        #     save_model(model.state_dict(), valid_performance[0], config.result_folder, config.learn_name)
...
def train_test(bench_iter, config):
    k = 0
    train_iter = [x for i, x in enumerate(bench_iter) if i % 5 != k]
    test_iter = [x for i, x in enumerate(bench_iter) if i % 5 == k]
    ...
    optimizer = torch.optim.Adam(params=model.parameters(), lr=config.lr, weight_decay=config.reg)
    criterion = nn.CrossEntropyLoss()
    # model.train()

    print('=' * 50 + 'Start Training' + '=' * 50)
    best_performance = train_m7G(train_iter, None, test_iter, model, optimizer, criterion, config, 0)
    # np.save("TMSC-m7G_roc.npy", roc_data)
    # np.save("TMSC-m7G_prc.npy", prc_data)
    print('=' * 50 + 'Train Finished' + '=' * 50)
...
def load_config(path_config_data = None):

    # path_config_data = '../result/m7G_model/config.pkl'
...
if __name__ == '__main__':
    ...
    '''load data'''
    ind_iter,bench_iter = load_data(config)
    ...
    if config.k_fold == -1:
        # train and test
        model, best_performance, last_test_metric = train_test(bench_iter, config)
        pass
    else:
        # k cross validation
        model, valid_performance_list = k_fold_CV(bench_iter, None, config)
